...
if __name__ == "__main__":
    ...
    if algChoice == 'ga':
        ...
        if pTune == 1:
            ...
            #f_name = str('res_ga ' +' ' +minSup+' ' + filePath+ ' '+ str(end).replace('.', '', 1) + '.txt')
            dataset_name = os.path.basename(filePath).split('.')[0]
            f_name = f"res_ga_{dataset_name}_s_{minSup}.txt"
        ...
        if pTune == 1:
            ...
            stamp = time.time()
            dataset_name = os.path.basename(filePath).split('.')[0]
            #f_name = f"res_ga_{dataset_name}_s{minSup}_{str(stamp).replace('.', '', 1)}.txt"
            ...
            dataset_name = os.path.basename(filePath).split('.')[0]
            f_name = f"res_pso_{dataset_name}_s_{minSup}.txt"
            sgp.write_file(wr_text, f_name, cfg.SAVE_RESULTS)
            print(wr_text)
    elif algChoice == 'prs':
        # PRS-GRAANK
        if pTune == 1:
            ...
            dataset_name = os.path.basename(filePath).split('.')[0]
            f_name = f"res_prs_{dataset_name}_s_{minSup}.txt"
            sgp.write_file(wr_text, f_name, cfg.SAVE_RESULTS)
            print(wr_text)
    elif algChoice == 'pls':
        # PLS-GRAANK
