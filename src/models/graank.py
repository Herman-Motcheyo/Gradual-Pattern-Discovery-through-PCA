...
def Trad(fileName):
    ...
    try:
        with open(fileName, 'r', encoding='utf-8') as f:
            # Spécifiez directement la virgule comme délimiteur
            reader = csv.reader(f, delimiter=',')
            temp = list(reader)
            
        # Vérifie si le fichier contient une ligne d'en-tête ou seulement des nombres
        if temp[0][0].replace('.', '', 1).isdigit() or temp[0][0].isdigit():
            # Fichier sans en-tête, retourne directement les données converties en float
            return [[float(temp[j][i]) for j in range(len(temp))] for i in range(len(temp[0]))]
        else:
            # Fichier avec une en-tête, analyse et structure les données
            if temp[0][1].replace('.', '', 1).isdigit() or temp[0][1].isdigit():
                ...
                # Ajoute les titres sous forme de liste
                title = [f"{i + 1} : {temp[0][i]}" for i in range(len(temp[0]))]
                data = [[float(temp[j][i]) for j in range(1, len(temp))] for i in range(len(temp[0]))]
                return title, data
...
def algorithm_gradual_gradual(file_name, min_sup):
    return Graank(Trad(file_name), min_sup, False)
